...
import cglversion3.conwaysGame;
 ...
 * @co_author R.V.Nikhil 2020501117
 * date - 29-12-2020
 * credits - Took help from my batch mate Sai Chandra for explaining the concept.
...
public class UserInterface {
	...
	**********************************************************************************************/
	...
	private Pane window;
	// decalring the 2d array list for future use.
	int [][] cells;
	...
	**********************************************************************************************/
			...
			for (int i =0; i <boardSizeWidth; i++){
				for(int j =0; j< boardSizeHeight; j++) {
					if(start.Board1.grid[i][j].check == true) {
						...
						item.setFill(Color.BROWN);
						evenCanvas.getChildren().add(item);
					}
				}
			}
	...
	public void runSimulation(){
		...
		// Your code goes here...
		/*
		 * continously calling the next generation and removing the existing generation and adding current
		 * generation to the window.
		 */
	...
	private boolean fileContentsAreValid() {
		...
		while (scanner_Input.hasNextLine()) {
			...
			// adding the input line to the array list.
			Line.add(inputLine);
		...
		/*
		 * initializing the cells with the size of line number and having the rows and coloumns in the array
		 * then iterating each element in the loop and adding the position of the live cell respectively.
		 */
		cells = new int [lineNumber][2];
		for(int i=0 ; i<lineNumber;i++) {
			String [] Cell1 = Line.get(i).split(" ");
			cells[i][0] = Integer.parseInt(Cell1[0]);
			cells[i][1] = Integer.parseInt(Cell1[1]);
		}
